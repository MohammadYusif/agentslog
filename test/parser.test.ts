...
import { parseSessionFile, normalizePath } from '../src/parser/claude-code.js';
...
describe('parseSessionFile', () => {
  it('accumulates input_tokens across ALL assistant messages but last_input_tokens from the last', async () => {
    const file = fixture([
      { type: 'user', sessionId: 's1', timestamp: '2026-01-01T00:00:00Z', cwd: 'C:\\proj', message: { role: 'user', content: 'hi' } },
      { type: 'assistant', sessionId: 's1', timestamp: '2026-01-01T00:00:01Z', message: { model: 'claude-opus-4-8', usage: { input_tokens: 100, output_tokens: 10, cache_read_input_tokens: 5, cache_creation_input_tokens: 2 }, content: [] } },
      { type: 'assistant', sessionId: 's1', timestamp: '2026-01-01T00:00:02Z', message: { model: 'claude-opus-4-8', usage: { input_tokens: 300, output_tokens: 20, cache_read_input_tokens: 7, cache_creation_input_tokens: 3 }, content: [] } },
    ]);
  ...
  it('skips corrupt JSON lines without throwing', async () => {
    const file = fixture([
      { type: 'user', sessionId: 's2', timestamp: '2026-01-01T00:00:00Z', message: { role: 'user', content: 'hi' } },
    ]);
    // Append a broken line + a valid assistant line.
    fs.appendFileSync(file, '{ this is not json \n');
    fs.appendFileSync(
      file,
      JSON.stringify({ type: 'assistant', sessionId: 's2', timestamp: '2026-01-01T00:00:01Z', message: { usage: { input_tokens: 50, output_tokens: 5 }, content: [] } }) + '\n'
    );
  ...
  it('extracts file_path only for Read/Write/Edit/Grep/Glob and normalizes separators', async () => {
    const file = fixture([
      { type: 'assistant', sessionId: 's3', timestamp: '2026-01-01T00:00:00Z', message: { content: [
  ...
  it('stores command for Bash/PowerShell truncated to 500 chars, with null file_path', async () => {
    ...
    const file = fixture([
      { type: 'assistant', sessionId: 's4', timestamp: '2026-01-01T00:00:00Z', message: { content: [
  ...
  it('assigns incrementing sequence_num starting at 0 across the session', async () => {
    const file = fixture([
      { type: 'assistant', sessionId: 's5', timestamp: '2026-01-01T00:00:00Z', message: { content: [
        { type: 'tool_use', id: 'a', name: 'Read', input: { file_path: '/x' } },
        { type: 'tool_use', id: 'b', name: 'Read', input: { file_path: '/y' } },
      ] } },
      { type: 'assistant', sessionId: 's5', timestamp: '2026-01-01T00:00:01Z', message: { content: [
  ...
  it('marks a tool call failed and captures error text when its result is_error', async () => {
    const file = fixture([
      { type: 'assistant', sessionId: 's6', timestamp: '2026-01-01T00:00:00Z', message: { content: [
        { type: 'tool_use', id: 'tool-x', name: 'Bash', input: { command: 'false' } },
      ] } },
      { type: 'user', sessionId: 's6', timestamp: '2026-01-01T00:00:01Z', message: { role: 'user', content: [
        { type: 'tool_result', tool_use_id: 'tool-x', is_error: true, content: 'Exit code 1: boom' },
      ] } },
    ]);
  ...
  it('uses the LAST ai-title event as the session title', async () => {
    const file = fixture([
      ...
      { type: 'user', sessionId: 's7', timestamp: '2026-01-01T00:00:00Z', message: { role: 'user', content: 'hi' } },
  ...
  it('aggregates files_touched read/write/edit counts', async () => {
    const file = fixture([
      { type: 'assistant', sessionId: 's8', timestamp: '2026-01-01T00:00:00Z', message: { content: [
  ...
  it('computes duration from first and last timestamps', async () => {
    const file = fixture([
      { type: 'user', sessionId: 's9', timestamp: '2026-01-01T00:00:00Z', message: { role: 'user', content: 'hi' } },
      { type: 'assistant', sessionId: 's9', timestamp: '2026-01-01T00:00:10Z', message: { content: [] } },
    ]);
  ...
  it('treats a canonical file (basename matches sessionId) as a top-level session', async () => {
    ...
    fs.writeFileSync(
      file,
      [
        { type: 'user', isSidechain: false, sessionId: 'parent-uuid', timestamp: '2026-01-01T00:00:00Z', message: { role: 'user', content: 'main' } },
        { type: 'assistant', isSidechain: false, sessionId: 'parent-uuid', timestamp: '2026-01-01T00:00:01Z', message: { usage: { input_tokens: 10, output_tokens: 2 }, content: [] } },
      ]
        .map((e) => JSON.stringify(e))
        .join('\n') + '\n',
      'utf-8'
  ...
  it('counts only real user text turns, not tool_result envelopes', async () => {
    const file = fixture([
      { type: 'user', sessionId: 's10', timestamp: '2026-01-01T00:00:00Z', message: { role: 'user', content: 'question one' } },
      { type: 'assistant', sessionId: 's10', timestamp: '2026-01-01T00:00:01Z', message: { content: [ { type: 'tool_use', id: 'q', name: 'Bash', input: { command: 'ls' } } ] } },
      { type: 'user', sessionId: 's10', timestamp: '2026-01-01T00:00:02Z', message: { role: 'user', content: [ { type: 'tool_result', tool_use_id: 'q', content: 'ok' } ] } },
      { type: 'user', sessionId: 's10', timestamp: '2026-01-01T00:00:03Z', message: { role: 'user', content: 'question two' } },
    ]);
